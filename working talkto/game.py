...
def is_valid_person(char_list, name):

    return name in char_list


def execute_talkto(person_name):
    if is_valid_person(characters, person_name) == False:

        return "That person doesn't exist.\n"
    global karma
    person = characters[person_name][0]
    text_lvl = characters[person_name][1]
    keys = len(person["dialog"]) - 1  # works out how many keys there are in a dictionary

    if text_lvl <= keys:  # if text_lvl is below the repeat text_lvl it will run
        if text_lvl == 0:  # if text_lvl shows intro hasn't been printed it will print
            text_lvl += 1
            characters[person_name][1] = text_lvl
            print(person["intro"])
        print(person["dialog"][text_lvl][1])
        print(person["dialog"][text_lvl][2])
        choice_input = input("Type 1 or 2 to choose your response.\nOr press enter to talk another time.\n")
        if choice_input == "1" or "2":  # checks if any text was entered
            if choice_input == "1":
                karma = karma - 1
                text_lvl += 1  # changes the text level
                characters[person_name][1] = text_lvl
                return person["dialog"][text_lvl][3]  # prints response 1
            if choice_input == "2":
                karma = karma + 1
                text_lvl += 1  # changes the text level
                characters[person_name][1] = text_lvl
                return person["dialog"][text_lvl][4]  # prints response 2
        else:  # if they type anything else or nothing it does move the text_lvl along
            return "You choose to speak another time...\n"

    else:
        return person["dialog"][text_lvl]

    # Karma needs to be added at the end of the game.
...
def execute_command(command, current):
    ...
    if command[0] == "toggle":
        ...
        if len(command) > 1:
            return execute_talkto(command[1])
        else:
            return "Talk to who?"
    else:
